...
- A flat-color, draggable and zoomable orthographic globe on the flights page (rendered on canvas with [d3-geo](https://d3js.org/d3-geo) over bundled [Natural Earth](https://www.naturalearthdata.com/) country data — no map tile service, satellite imagery, or API key needed). Every flight renders as a thin great-circle line with a small dot at each airport. Airport coordinates come from a bundled ICAO/IATA lookup — no coordinates to enter manually. Routes are unlabeled by default to avoid clutter; clicking one highlights it and shows its two airports' city/code labels
