...
- A flat-color, draggable and zoomable orthographic globe on the flights page (rendered on canvas with [d3-geo](https://d3js.org/d3-geo) over bundled [Natural Earth](https://www.naturalearthdata.com/) country boundaries and [GeoNames](https://www.geonames.org/) city data — no map tile service, satellite imagery, or API key needed). Major cities are labeled, with more appearing as you scroll to zoom in. Every flight renders as a thin great-circle line with a small dot at each airport. Airport coordinates come from a bundled ICAO/IATA lookup — no coordinates to enter manually. Routes are unlabeled by default to avoid clutter; clicking one highlights it on the globe (with its two airports' city/code labels) and highlights the matching entry in the flight log below, and vice versa
