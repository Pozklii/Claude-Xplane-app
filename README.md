...
- A rotatable 3D globe on the flights page with a neon color scheme, rendered with [MapLibre GL JS](https://maplibre.org/maplibre-gl-js/docs/) over free, no-API-key [OpenStreetMap](https://www.openstreetmap.org/copyright) vector tiles from [OpenFreeMap](https://openfreemap.org/) (swap `STYLE_URL` in `flight-globe.tsx` for a MapTiler/Mapbox satellite style if you have a key — the neon recolor in `applyNeonStyle` is schema-based, keying off each layer's OpenMapTiles `source-layer`/type rather than its specific ID, so it keeps working across most vector styles). Flight routes and airport markers are drawn with [deck.gl](https://deck.gl/)'s `ArcLayer` and `ScatterplotLayer`, composited into the MapLibre render pass via `@deck.gl/mapbox`'s `MapboxOverlay` (`interleaved: true`), with a glow effect (a wide, low-opacity layer beneath a thin, bright one) on both. Clicking a route highlights it on the globe (with its two airports' city/code labels) and highlights the matching entry in the flight log below, and vice versa. Airport coordinates come from a bundled ICAO/IATA lookup — no coordinates to enter manually.
