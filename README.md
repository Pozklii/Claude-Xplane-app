...
- A landing page (`/`) with a dark aviation/instrument-panel look — brushed-metal panel seams, HUD-style corner brackets, converging livery lines feeding into a generatively-drawn turbofan engine (spinning blades via CSS animation) — and a live example globe using the same `FlightGlobe` component as the flights page. The globe tours an endless stream of generated example flights: every 7 seconds it generates a new one and flies onto it, keeping the last six drawn. They're built from a table of real airlines on routes they're known for, with aircraft types they typically fly there, plus a few private/GA hops (`src/lib/example-flights/routes.ts`); the generator (`src/lib/example-flights/generate.ts`) randomizes direction, aircraft, date (anywhere from the visitor's own today back ~18 months — dated in their browser, since the page itself is prerendered) and notes (two sentences from different phases of flight, drawn from a pool of templates in `notes.ts` that avoids phrasings already on screen and airline-only details on private flights), and estimates flight time from the real great-circle distance and the aircraft's cruise speed. Airport coordinates come from the bundled OurAirports lookup, and routes crossing the antimeridian are skipped. A summary card (the same one as the flights page, minus thumbnail and close) shows its route, distance, flight time, date, aircraft, airline and notes. The tour can be paused. The landing globe is display-only (`FlightGlobe`'s `interactive={false}`: MapLibre's handlers off and `pointer-events: none`), so it can't be dragged, zoomed or clicked, and scrolling over it scrolls the page; the tour alone drives it. The flights page's globe stays fully interactive. The artwork hero fills the top three quarters of the viewport and a plain black band with the feature blurbs the bottom quarter (a 3:1 flex split). All of the decorative SVG chrome is computed once at module load in `page.tsx` (deterministic, no client JS needed), and it's a deliberately single dark theme rather than adapting to light/dark preference.
