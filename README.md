...
- A flat-color, pannable and deeply zoomable world map on the flights page (rendered on canvas with [d3-geo](https://d3js.org/d3-geo)'s equirectangular projection over bundled [Natural Earth](https://www.naturalearthdata.com/) country boundaries and nearly 20,000 [GeoNames](https://www.geonames.org/) cities down to ~20,000 population — no map tile service, satellite imagery, or API key needed). The visible area is pre-rendered to a small off-screen bitmap around the current view that's simply panned and zoomed, so dragging stays smooth and memory use stays bounded no matter how far in you zoom. Major cities are labeled by default, with progressively smaller ones appearing as you scroll to zoom in. Every flight renders as a thin great-circle line with a small dot at each airport (correctly split at the antimeridian for trans-Pacific routes). Airport coordinates come from a bundled ICAO/IATA lookup — no coordinates to enter manually. Routes are unlabeled by default to avoid clutter; clicking one highlights it on the map (with its two airports' city/code labels) and highlights the matching entry in the flight log below, and vice versa
