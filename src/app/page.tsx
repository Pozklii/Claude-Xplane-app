...
const nacelleRivets = Array.from({ length: RIVET_COUNT }, (_, i) =>
  pt(ENGINE_CX, ENGINE_CY, NACELLE_R, (360 / RIVET_COUNT) * i),
);

const statorVanes = Array.from({ length: STATOR_COUNT }, (_, i) => {
  const theta0 = (360 / STATOR_COUNT) * i;
  return {
    q1: pt(ENGINE_CX, ENGINE_CY, BLADE_INNER_R - 4, theta0),
    q2: pt(ENGINE_CX, ENGINE_CY, STATOR_R, theta0),
  };
});

const bladesData = Array.from({ length: BLADE_COUNT }, (_, i) => {
  const theta = (360 / BLADE_COUNT) * i;
  const p1 = pt(ENGINE_CX, ENGINE_CY, BLADE_INNER_R, theta - HALF_WIDTH_DEG);
  const p2 = pt(ENGINE_CX, ENGINE_CY, BLADE_OUTER_R, theta - HALF_WIDTH_DEG + SWEEP_DEG);
  const p3 = pt(ENGINE_CX, ENGINE_CY, BLADE_OUTER_R, theta + HALF_WIDTH_DEG + SWEEP_DEG);
  const p4 = pt(ENGINE_CX, ENGINE_CY, BLADE_INNER_R, theta + HALF_WIDTH_DEG);
  const rootPt = pt(ENGINE_CX, ENGINE_CY, BLADE_INNER_R - 1, theta);
  return {
    i,
...
const hubBolts = Array.from({ length: HUB_BOLT_COUNT }, (_, i) =>
  pt(ENGINE_CX, ENGINE_CY, HUB_R - 6, (360 / HUB_BOLT_COUNT) * i + 15),
);

const cowlSheenD = arcPath(ENGINE_CX, ENGINE_CY, CASE_OUTER_R, -55, 15);
      ...
      <section className={styles.hero}>
        ...
        <div className={styles.liveryFx} aria-hidden="true">
          <svg viewBox="0 0 1440 900" preserveAspectRatio="xMidYMid meet">
            ...
            {/* Turbofan intake: recessed nacelle, riveted cowl, fan-case
                rings, static stator vanes, a ring of spinning swept blades
                with lit leading edges and root bolts, and a shaded spinner
                hub — the livery lines below read as airflow feeding in. */}
            <g>
              <circle cx={ENGINE_CX} cy={ENGINE_CY} r={NACELLE_R} fill="url(#engineRecess)" />
              <circle cx={ENGINE_CX} cy={ENGINE_CY} r={NACELLE_R} fill="none" stroke="#2c4d68" strokeWidth="1.5" opacity="0.5" />
              <g fill="#a9c0d0" opacity="0.4">
                {nacelleRivets.map(([x, y], i) => (
                  <circle key={i} cx={x.toFixed(1)} cy={y.toFixed(1)} r="1.4" />
                ))}
              </g>
              <circle cx={ENGINE_CX} cy={ENGINE_CY} r={CASE_OUTER_R} fill="none" stroke="#5a9fd4" strokeWidth="2" opacity="0.4" />
              <path d={cowlSheenD} fill="none" stroke="#dcedf7" strokeWidth="2" strokeLinecap="round" opacity="0.45" />
              <circle cx={ENGINE_CX} cy={ENGINE_CY} r={CASE_INNER_R} fill="none" stroke="#8fd0f5" strokeWidth="1.5" opacity="0.5" />
              ...
              <circle cx={ENGINE_CX} cy={ENGINE_CY} r={HUB_R} fill="url(#engineHub)" stroke="#b9c8d4" strokeWidth="1" opacity="0.9" />
              <g fill="#0e2434" opacity="0.55">
                {hubBolts.map(([x, y], i) => (
                  <circle key={i} cx={x.toFixed(1)} cy={y.toFixed(1)} r="1.6" />
                ))}
                <circle cx={ENGINE_CX} cy={ENGINE_CY} r="2.4" />
              </g>
            </g>
